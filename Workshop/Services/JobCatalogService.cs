...
public sealed class JobCatalogService
{
    ...
    public IEnumerable<string> Categories => CategoryColors.Keys;
    ...
    public async Task SaveSettingsAsync()
    {
        ...
        settings.ServiceCategoryHierarchy = NormalizeCategoryHierarchy(ServiceCategoryHierarchy);
    ...
    private static Dictionary<string, List<string>> BuildServiceCategoryHierarchy(CatalogSettings settings, IEnumerable<JobDefinition> jobs)
    {
        var fromSettings = NormalizeCategoryHierarchy(settings.ServiceCategoryHierarchy);
        if (fromSettings.Count > 0)
            return fromSettings;

        var fallback = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
        foreach (var job in jobs)
        {
            var category1 = NormalizeCategory1(job.Category);
            var category2 = NormalizeCategory2(job.Category2);
            if (!fallback.TryGetValue(category1, out var set))
            {
                set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                fallback[category1] = set;
            }
    ...
    {
        ServiceCategoryHierarchy = NormalizeCategoryHierarchy(hierarchy);
    }
    ...
    private static Dictionary<string, List<string>> NormalizeCategoryHierarchy(Dictionary<string, List<string>>? hierarchy)
    {
        ...
        foreach (var entry in hierarchy)
        {
            ...
            var category2Items = (entry.Value ?? new List<string>())
